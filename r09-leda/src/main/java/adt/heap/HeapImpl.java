...
public class HeapImpl<T extends Comparable<T>> implements Heap<T> {
	...
	private void heapify(int position) {
		...
		if (this.left(position) < this.size()
				&& (this.comparator.compare(this.heap[position], this.heap[this.left(position)]) > 0))
			index_max = this.left(position);
		else
			index_max = position;

		if (this.right(position) < this.size()
				&& (this.comparator.compare(this.heap[index_max], this.heap[this.right(position)]) > 0))
			index_max = this.right(position);

		if (index_max != position) {
			Util.swap(this.heap, position, index_max);
			heapify(index_max);
		}
	...
	public T[] heapsort(T[] array) {
		Comparator<T> comparatorAux = this.comparator;
		...
		for (int i = 0; i < array.length; i++) 
			listAux.add(this.extractRootElement());
		
		this.heap = (T[]) (new Comparable[INITIAL_SIZE]);
		this.setComparator(comparatorAux);
		
		return listAux.toArray((T[]) new Comparable[0]);
	}
